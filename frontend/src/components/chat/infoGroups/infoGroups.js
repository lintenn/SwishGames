...
export const infoGroup = ( myGroups, id, participantes, userAct, setGroup, setReceptor, setConexion ) => {
  ...
  viewInfoGroup( groupAct, admin, participantes, setGroup, userAct, setReceptor, setConexion );

};

function viewInfoGroup( groupAct, admin, participantes, setGroup, userAct, setReceptor, setConexion ) {

  Swal.fire({
    html: `<div class="max-tamaño-swal-Chat" style="background-color: #f0eeee">${showInfoGroups( groupAct, admin )}</div>`,
    background: '#f0eeee',
    ...
    didOpen: () => {
      ...
      addClickButton( groupAct, admin, participantes, setGroup, userAct, setReceptor, setConexion );
...
function showInfoGroups( group, admin ) {
...
function addClickButton( groupAct, admin, participantes, setGroup, userAct, setReceptor, setConexion ) {
  ...
  if ( groupAct.descripcion !== '' ) {
    ...
    botonCopiarDescripcion.addEventListener( 'click', ( event ) => {
      ...
      copyInfo( 'descripcion', groupAct, participantes, userAct, setReceptor, setConexion, setGroup );
  ...
  botonCopiarNombre.addEventListener( 'click', ( event ) => {
    ...
    event.preventDefault();
    copyInfo( 'nombre', groupAct, participantes, userAct, setReceptor, setConexion, setGroup );
  ...
  document.querySelector( 'button[name="verParticipantes"]' ).addEventListener( 'click', ( event ) => {
    ...
    event.preventDefault();
    viewParticipantsGroup( groupAct, admin, participantes, setGroup, userAct, setReceptor, setConexion );
  ...
  if ( admin ) {
    ...
    botonEditarDescripcion.addEventListener( 'click', ( event ) => {
      ...
      event.preventDefault();
      editInfo( 'descripcion', groupAct, participantes, setGroup, userAct, setReceptor, setConexion );
    ...
    botonEditarNombre.addEventListener( 'click', ( event ) => {
      ...
      event.preventDefault();
      editInfo( 'nombre', groupAct, participantes, setGroup, userAct, setReceptor, setConexion );
    ...
    botonEditarImagen.addEventListener( 'click', ( event ) => {
      ...
      event.preventDefault();
      editInfo( 'imagen', groupAct, participantes, setGroup, userAct, setReceptor, setConexion );
